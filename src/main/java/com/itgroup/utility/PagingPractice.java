...
public class PagingPractice {  //shopping 교안 237
    //페이징 관련 변수
    private int totalCount = 0 ; //총 레코드 건수  33
    private int totalPage = 0 ; //전체 페이지 수  totalCount / pageSize + 1
    ...
    public PagingPractice(String _pageNumber, String _pageSize, int totalCount, String url, String mode, String keyword) {
        // 여기 코딩해 오세요.
        ...
        this.beginRow = (pageNumber-1) * pageSize + 1; //현재 페이지의 시작 행
        //this.beginRow = pageNumber * pageSize - (pageSize-1);  같은 수식.

        this.endRow = Math.min(this.beginRow + pageSize - 1, totalCount);//현재 페이지의 끝 행
        //this.endRow = pageNumber * pageSize;
        //if(this.endRow > this.totalCount) {this.endRow = this.totalCount}
        this.beginPage = Math.max(((pageNumber-1) / pageSize)  * pageSize + 1 , 1); //페이징 처리 시작 페이지 번호
        this.endPage = Math.min(beginPage + pageCount -1 , totalPage); //페이징 처리 끝 페이지 번호




        int maxPages = (int) Math.ceil((double)totalCount / pageSize);
        this.pageCount = pageSize ; //보여줄 페이지 링크 수
